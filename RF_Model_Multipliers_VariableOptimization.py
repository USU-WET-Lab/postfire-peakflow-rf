...
# metric_short = 'Peak'
# scenario = 'modelbounds_17_seed99'
withholding = '90_10'
# postfire = 'PostFire'

print(withholding)#, scenario)

working = 'C:\\Users\\A02343538\\Box\\MyResearch\\Chap3'
workingPath = '{}\\RandomForest\\RFModels\\UpdatedModelRuns_Spring26\\Peak\\Full'.format(working)
data_file = '{}\\RF_AttributeTable_ARI1_Peak_modelbounds_Full_ForUpdatedModelRuns_trimmed.csv'.format(workingPath)
data_og = pd.read_csv(data_file)

data_og = data_og[(data_og.DRAIN_SQKM >= 50)]
data_og = data_og[(data_og.MTBS_burnedarea_per >= 20)]
data_og = data_og[data_og.burned_storm_depth_per >= 70]
data_og = data_og[data_og.DaysSinceFire <= 1095]
...
# for x in range(0, 1):
    ...
    # loop to remove the least important variable from last round
    data_cols = len(data_og.columns.to_list()) -2 # subtract 2 for GAGE_ID and multiplier
    data = data_og

    for i in range(0,data_cols):
        data_length = len(data.columns.to_list()) -2

        if not os.path.exists('{}\\VariableOptimization\\Seed_{}'.format(workingPath, x)):
            os.mkdir('{}\\VariableOptimization\\Seed_{}'.format(workingPath, x))
        ...
        stats_df.to_csv('{}\\VariableOptimization\\Seed_{}\\Stats_Vars{}.csv'.format(workingPath, x, data_length))
        ...
        importances_df.to_csv('{}\\VariableOptimization\\Seed_{}\\Importances_Vars{}.csv'.format(workingPath,x, data_length))
        ...
        # plt.savefig('{}\\RandomForest\\RFModels\\StormPercentileVersions\\Variables\\{}\\{}\\{}\\Seed_{}\\Vars_{}\\Residuals.png'.format(workingPath, postfire,metric_short,witholding, x,data_length))
        # # plt.show()
        # plt.close()
        #
        ...
        # plt.savefig('{}\\RandomForest\\RFModels\\StormPercentileVersions\\Variables\\{}\\{}\\{}\\Seed_{}\\Vars_{}\\Predicted.png'.format(workingPath, postfire, metric_short,witholding,x, data_length))
        # # plt.show()
        # plt.close()
        ...
        #     # display.plot()
        #     plt.title(col)
        #     plt.savefig('{}\\RandomForest\\RFModels\\Optimization\\Variables\\PostFireMultipliers\\{}\\{}\\Seed_{}\\Vars_{}\\PD\\pd_{}.png'.format(workingPath, metric_short, witholding, x,data_length, col))
        #     plt.close()

        # remove the least important variable
        least_important = features_sorted[-1]
        print(least_important)
        data = data.drop(least_important, axis = 1)
